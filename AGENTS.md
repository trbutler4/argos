...
installed host instances. `vm list` reads local VM state, `vm create` writes local state/workdir scaffolds, and `vm start`/`vm stop` run the local microVM process from that state. A first microvm.nix QEMU runner-package prototype builds
