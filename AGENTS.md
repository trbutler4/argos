...
installed host instances. `vm list` reads local VM state, `vm create` writes local state/workdir scaffolds, `vm start`/`vm stop` run the local microVM process from that state, and `vm console` attaches to the tmux-backed VM console session. A first microvm.nix QEMU runner-package prototype builds
