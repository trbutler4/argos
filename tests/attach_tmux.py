...
def main():
    ...
    with tempfile.TemporaryDirectory(prefix="argos-attach-", dir=os.environ.get("XDG_RUNTIME_DIR")) as temp:
        ...
        try:
            ...
            prefix = [str(BINARY), "sessions", "attach"]
            ...
            wait(lambda: attached().get(str(actor[0].pid)) == source, "Argos did not sessions attach from a plain terminal")
            ...
            assert status != "0"
            assert set(attached().values()) == {target}, "ambiguous sessions attach moved a client"
