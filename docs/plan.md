...
### Slice 8: local VM state listing scaffold

Implemented interface: `argos vm list [--json] [--state-dir PATH]`. This is
read-only host-local VM lifecycle scaffolding. It reads versioned JSON VM records
from the host Argos state directory under `vms/*.json`, returns a stable schema
versioned snapshot, sorts records by ID, treats missing state as an empty list,
and rejects malformed state rather than starting or creating VMs.

Boundaries: no VM create/start/stop, no microvm process supervision, no SSH into
guests, no systemd units and no state migration exist in this slice. Future
lifecycle commands should write this state shape after they have successfully
